...
--  - Require 2-person approval for super_admin role changes
--  - Keep enforcement server-side (DB), not UI-only
...
CREATE POLICY admin_audit_log_audit_read_select
...
CREATE TABLE IF NOT EXISTS public.admin_role_change_requests (
  ...
  requested_role_keys text[] NOT NULL,
  note text,
...
--  - require approval request for super_admin changes
