-- Security hardening refresh: deny-by-default for public RPC EXECUTE privileges.
--
-- Why this exists:
-- `REVOKE EXECUTE ON ALL FUNCTIONS ...` only affects functions that exist at the
-- time the migration runs. Any later migration that creates/replaces a function
-- can accidentally re-expose EXECUTE to PUBLIC (and therefore anon/authenticated).
--
-- This migration re-applies the allowlist as the last step in the migration
