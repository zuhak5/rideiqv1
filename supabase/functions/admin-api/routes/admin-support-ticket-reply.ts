...
import { adminSupportTicketReplyBodySchema } from '../../_shared/schemas.ts';
...
export async function handle(req: Request, ctx: any): Promise<Response> {
  ...
  const body = await validateJsonBody(req, ctx, adminSupportTicketReplyBodySchema);
  if (!body.ok) return body.res;

  const supabase = createUserClient(req);
  const { ticket_id, message, attachments } = body.data;

  const { data, error } = await supabase.rpc('admin_support_ticket_post_message_v1', {
  ...
  if (error) {
    ctx?.error?.('admin_support.ticket_reply.rpc_error', { error: error.message });
