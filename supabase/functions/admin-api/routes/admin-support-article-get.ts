...
import { adminSupportArticleGetQuerySchema } from '../../_shared/schemas.ts';
...
export async function handle(req: Request, ctx: any): Promise<Response> {
  ...
  const q = validateQuery(req, ctx, adminSupportArticleGetQuerySchema);
  if (!q.ok) return q.res;

  const supabase = createUserClient(req);
  const { data, error } = await supabase.rpc('admin_support_article_get_v1', { p_id: q.data.id });

  if (error) {
    ctx?.error?.('admin_support.article_get.rpc_error', { error: error.message });
