...
async function fetchMyStatementSummary(dateFrom: string, dateTo: string): Promise<StatementSummaryRow> {
  const { data, error } = await supabase.rpc('merchant_settlement_statement_summary_v1', {
...
async function fetchMyStatementEntries(dateFrom: string, dateTo: string): Promise<StatementEntryRow[]> {
  const { data, error } = await supabase.rpc('merchant_settlement_statement_entries_v1', {
    p_start: isoStart(dateFrom),
    p_end: isoEndExclusive(dateTo),
    p_limit: 1000,
    p_offset: 0,
...
function makeIdempotencyKey(prefix: string): string {
  ...
  const uuid = typeof g?.crypto?.randomUUID === 'function' ? g.crypto.randomUUID() : `${Date.now()}-${Math.random()}`;
  return `${prefix}:${uuid}`;
}

async function fetchMyAccount(): Promise<AccountRow> {
  const { data, error } = await supabase.rpc('merchant_settlement_get_my_account_v1');
  if (error) throw error;
  const row = Array.isArray(data) ? data[0] : data;
...
async function fetchMyEntries(): Promise<EntryRow[]> {
  const { data, error } = await supabase.rpc('merchant_settlement_list_entries_v1', { p_limit: 100, p_offset: 0 });
  if (error) throw error;
  return (Array.isArray(data) ? data : []) as EntryRow[];
}

async function fetchMyPaymentRequests(): Promise<RequestRow[]> {
  const { data, error } = await supabase.rpc('merchant_settlement_list_payment_requests_v1', { p_limit: 50, p_offset: 0 });
  if (error) throw error;
  return (Array.isArray(data) ? data : []) as RequestRow[];
}

async function fetchMyPayoutRequests(): Promise<RequestRow[]> {
  const { data, error } = await supabase.rpc('merchant_settlement_list_payout_requests_v1', { p_limit: 50, p_offset: 0 });
  if (error) throw error;
  return (Array.isArray(data) ? data : []) as RequestRow[];
}
...
export default function MerchantSettlementPage() {
  ...
  const stmtSummaryQ = useQuery<StatementSummaryRow, Error>({
    queryKey: ['merchant_settlement_statement_summary', stmtFrom, stmtTo],
  ...
  const refresh = React.useCallback(() => {
    void qc.invalidateQueries({ queryKey: ['merchant_settlement_account'] });
    void qc.invalidateQueries({ queryKey: ['merchant_settlement_entries'] });
    void qc.invalidateQueries({ queryKey: ['merchant_settlement_payment_requests'] });
    void qc.invalidateQueries({ queryKey: ['merchant_settlement_payout_requests'] });
  }, [qc]);

  const paymentMut = useMutation({
    mutationFn: async () => {
      const amount = parseIQDInput(payAmount);
      const method = payMethod.trim();
      const reference = payRef.trim() || null;
      const { error } = await supabase.rpc('merchant_settlement_request_payment_v1', {
        p_amount_iqd: amount,
        p_method: method,
        p_reference: reference,
        p_idempotency_key: makeIdempotencyKey('merchant-pay'),
      });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const payoutMut = useMutation({
    mutationFn: async () => {
      const amount = parseIQDInput(payoutAmount);
      const method = payoutMethod.trim();
      const reference = payoutRef.trim() || null;
      const { error } = await supabase.rpc('merchant_settlement_request_payout_v1', {
        p_amount_iqd: amount,
        p_method: method,
        p_reference: reference,
        p_idempotency_key: makeIdempotencyKey('merchant-payout'),
      });
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="card p-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <div className="text-base font-semibold">Merchant settlement</div>
            <div className="text-sm text-gray-500 mt-1">
              For COD orders, drivers collect cash from customers and the platform tracks what is owed to you here.
            </div>
          </div>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                downloadCsv(`merchant_statement_${stmtFrom}_${stmtTo}.csv`, lines.join('\n'));
              } catch (err: any) {
      ...
      <div className="card p-5">
        ...
        <div className="mt-1 text-sm text-gray-500">
          Request a payout when the platform owes you (positive balance), or report a payment when you settle an amount you owe (negative balance).
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-xl border p-4">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3">
              ...
              <button className="btn btn-primary" type="button" onClick={() => payoutMut.mutate()} disabled={payoutMut.isPending || balance <= 0}>
                {payoutMut.isPending ? 'Submitting…' : 'Submit payout request'}
              </button>
              {balance <= 0 ? <div className="text-xs text-gray-500">Balance must be positive to request a payout.</div> : null}
            </div>
          </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder={owes ? String(absBal) : 'e.g. 50,000'}
                />
              </div>
              <div>
                <div className="text-xs text-gray-600">Method</div>
                <input
                  ...
                  placeholder="cash / office / transfer"
                />
              </div>
              <div>
                <div className="text-xs text-gray-600">Reference (optional)</div>
                  ...
                  <tbody>
                    {payoutReqQ.data.slice(0, 10).map((r) => (
                      <tr key={r.id} className="border-t">
                        ...
                        <td className="py-2 font-semibold">{formatIQD(r.amount_iqd)}</td>
                        <td className="py-2">
...
function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`inline-flex items-center px-2 py-1 rounded-lg border text-xs ${cls}`}>{s}</span>;
}
