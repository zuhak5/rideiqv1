...
export default function MerchantPayoutsPage() {
  ...
  useEffect(() => {
    void (async () => {
      const { data: account } = await supabase.rpc('merchant_settlement_get_my_account_v1');
      const accountRow = Array.isArray(account) ? account[0] : null;
      setBalance(accountRow?.balance_iqd ?? null);

      const { data: list } = await supabase.rpc('merchant_settlement_list_entries_v1', { p_limit: 20, p_offset: 0 });
  ...
  return (
    <Stack spacing={2}>
      ...
      <Paper sx={{ p: 2 }}>
        <Typography variant="h6">Settlement entries</Typography>
